$(document).ready(function() {
	...
	$.ajax({
		...
		url: 'api/get_count_main_data',

		success: function(data) {
			mainData = data;

			$('.landing_count').html(mainData.consumerCount);
			$('.call_count').html(mainData.callCount);
			$('.conversion_count').html(mainData.conversionCount);
			$('.client_count').html(mainData.clientCount);
			...
			if ($('.pages_dashboard').length) {
				...
				if ($('.ld-widget-main__chart').length) {
					Morris.Line({
						...
						labels: ['Landing', 'Calls', 'Conversion'],
				...
				if ($('.ld-widget-side__chart').length) {
					Morris.Donut({
						...
						data: [
							{label: "Landing", value: mainData.consumerCount},
							{label: "Calls", value: mainData.callCount},
							{label: "Conversions", value: mainData.conversionCount}
						],
